...
- `SLTI` - places `1` in `rd` if `rs1` is less than the sign-extended immediate when both are signed numbers else `0`. For unsigned integers, `SLTIU` is used, it sets `rd` to `1` if `rs1` is less than the sign-extended immediate, but in this case the signed-extended immediate is treated as as unsigned integer else  `rd` is set to `0`.
